...
class Visualizer:
    ...
    def __init__(self, args):
        ...
        #print("&*******************************************&******************************&")
        self.lastFrames = deque(maxlen=30)
    ...
    def publishIdentity(self, identity):
    	  print("*********************PUBLISHING*************************")
    	  self.client.publish("topic/register", identity)
    	
    def registerAttendance(self, name):
        print(self.lastFrames)
        if(name not in self.lastFrames):
            self.serialcount = self.serialcount + 1
            self.publishIdentity(str(self.serialcount)+"|"+name+"|"+str(self.date)+"|"+time.strftime('%X')+"|00:00")
            self.lastFrames.extend([name])
    ...
    def should_stop_display(self):
        ...
        #increment date by 1 when 'n' is pressed (for demo)
        if (key == 110):
